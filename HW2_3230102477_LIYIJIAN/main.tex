...
$f''(\xi(x))_{\max} = \frac{1}{2x}_{\max} = \frac{1}{2}$.
...
$f[0,1,3] = 1,f[1,3,4] = 2$\\
$f[0,1,3,4] = \frac{1}{4}$. So $p_3(f;x) = 5 - 2x + x(x-1) + \frac{1}{4}x(x-1)(x-3)$.
\subsection*{IV.b}
...
$ f[0,1,1,1,2,2] = 30$
\subsection*{V.b}
$f^{(5)}(\xi) = \frac{7!}{2*5!} \xi^2 = 30$, so $\xi^2 = \frac{10}{7}$, so $\xi = \sqrt{\frac{10}{7}}$
...
\]
Let $Q(x) = \hat{p}_n(x) - p(x)$, then in the $n+1$ extreme points ($\in [-1,1]$) of $T_n(x)$,like the proof in Chebyshev Theorem
