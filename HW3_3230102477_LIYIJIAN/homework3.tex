...
$=$
\[
\frac{(t_{i+2}-x)^2_+ -(t_{i+1}-x)^2_+}{(t_{i+2}-t_{i+1})(t_{i+2}-t_i)}
-\frac{(t_{i+1}-x)^2_+ -(t_{i}-x)^2_+}{(t_{i+1}-t_{i})(t_{i+2}-t_i)}-
\frac{(t_{i+1}-x)^2_+ -(t_{i}-x)^2_+}{(t_{i+1}-t_{i})(t_{i+1}-t_{i-1})}
+\frac{(t_{i}-x)^2_+ -(t_{i-1}-x)^2_+}{(t_{i}-t_{i-1})(t_{i+1}-t_{i-1})}
\]
When $x>t_{i+2}$ $LHS = 0 = RHS$.\\
When $x\leqslant t_{i-1}$ 
\[
LHS = \frac{(t_{i+2}-x)^2 -(t_{i+1}-x)^2}{(t_{i+2}-t_{i+1})(t_{i+2}-t_i)}
-\frac{(t_{i+1}-x)^2 -(t_{i}-x)^2}{(t_{i+1}-t_{i})(t_{i+2}-t_i)}-
\frac{(t_{i+1}-x)^2 -(t_{i}-x)^2}{(t_{i+1}-t_{i})(t_{i+1}-t_{i-1})}
+\frac{(t_{i}-x)^2 -(t_{i-1}-x)^2}{(t_{i}-t_{i-1})(t_{i+1}-t_{i-1})}
\]
$=$
\[
\frac{t_{i+2}+t_{i+1}-2x}{t_{i+2}-t_i}
-\frac{t_{i+1}+t_{i}-2x}{t_{i+2}-t_i}-
\frac{t_{i+1}+t_{i}-2x}{t_{i+1}-t_{i-1}}
+\frac{t_{i}+t_{i-1}-2x}{t_{i+1}-t_{i-1}} = 1-1 = 0
\]
When $t_{i-1}<x\leqslant t_{i}$ 
\[
LHS =
\frac{t_{i+2}+t_{i+1}-2x}{t_{i+2}-t_i}
-\frac{t_{i+1}+t_{i}-2x}{t_{i+2}-t_i}-
\frac{t_{i+1}+t_{i}-2x}{t_{i+1}-t_{i-1}}
+\frac{(t_{i}-x)^2}{(t_{i}-t_{i-1})(t_{i+1}-t_{i-1})}
\]
$=$
\[
\frac{2x - t_{i-1}-t_{i}}{t_{i+1}-t_{i-1}} +\frac{(t_{i}-x)^2}{(t_{i}-t_{i-1})(t_{i+1}-t_{i-1})}
=
 \frac{(t_{i-1}-x)^2}{(t_{i}-t_{i-1})(t_{i+1}-t_{i-1})} = RHS
\]
When $t_{i}<x\leqslant t_{i+1}$ 
\[
LHS =
\frac{t_{i+2}+t_{i+1}-2x}{t_{i+2}-t_i}
-\frac{(t_{i+1}-x)^2 }{(t_{i+1}-t_{i})(t_{i+2}-t_i)}-
\frac{(t_{i+1}-x)^2}{(t_{i+1}-t_{i})(t_{i+1}-t_{i-1})}
\]
at $t_{i+1}$ $LHS|_{t_{i+1}} = \frac{t_{i+2}-t_{i+1}}{t_{i+2}-t_i} = RHS|_{t_{i+1}}$\\
Besides $\frac{d}{dx}LHS = \frac{2(t_{i+1}-x) }{(t_{i+1}-t_{i})(t_{i+2}-t_i)}+
\frac{2(t_{i+1}-x)}{(t_{i+1}-t_{i})(t_{i+1}-t_{i-1})}-\frac{2}{t_{i+2}-t_i} = 
\frac{-2}{(t_{i+1}-t_{i-1})(t_{i+1}-t_{i})}+
\frac{-2}{(t_{i+2}-t_{i})(t_{i+1}-t_{i})} = \frac{d}{dx}RHS$ \\
So $LHS = RHS,t_{i}<x\leqslant t_{i+1}$.\\
When $t_{i+1}<x\leqslant t_{i+2}$ 
\[
LHS =
\frac{(t_{i+2}-x)^2}{(t_{i+2}-t_{i+1})(t_{i+2}-t_i)} = RHS
\]
Then $LHS = RHS$.

\section*{VII. Scaled integral of B-splines}
By the derivatives of B-splines
\[
\frac{d}{dx}B_i^{n+1}(x) = \frac{(n+1)B^{n}_i(x)}{t_{i+n}-t_{i-1}}-
\frac{(n+1)B^{n}_{i+1}(x)}{t_{i+n+1}-t_{i}}
\]
Since the support of $B_i^{n+1}(x)$ is finite
$\Rightarrow$

\[
\int_{\mathbb{R}}\frac{d}{dx}B_i^{n+1}(x)dx = \int_{\mathbb{R}}\frac{(n+1)B^{n}_i(x)}{t_{i+n}-t_{i-1}}dx-
\int_{\mathbb{R}}\frac{(n+1)B^{n}_{i+1}(x)}{t_{i+n+1}-t_{i}}dx
 = 0
\]
$\Rightarrow$
\[\frac{1}{t_{i+n}-t_{i-1}}\int_{t_{i-1}}^{t_{i+n}}B^{n}_i(x)dx=
\frac{1}{t_{i+n+1}-t_{i}}\int_{t_{i}}^{t_{i+n+1}}B^{n}_{i+1}(x)dx
\]
So the scaled integral of B-splines are independent of its index $i$. 
...
=\frac{[x_{i+1},x_{i+2}]x^4 - [x_{i},x_{i+1}]x^4}{x_{i+2}-x_{i}}
\]
$=$
\[
\frac{x_{i+2}^4-x_{i+1}^4}{(x_{i+2}-x_{i})(x_{i+2}-x_{i+1})} - 
\frac{x_{i+1}^4-x_{i}^4}{(x_{i+2}-x_{i})(x_{i+1}-x_{i})} 
= \frac{(x_{i+2}^2+x_{i+1}^2)(x_{i+2}+x_{i+1})}{x_{i+2}-x_{i}} - 
\frac{(x_{i+1}^2+x_{i}^2)(x_{i+1}+x_{i})}{x_{i+2}-x_{i}} 
\]
$=$
\[
\frac{(x_{i+2}^2+x_{i+1}^2)(x_{i+2}+x_{i+1})-(x_{i+1}^2+x_{i}^2)(x_{i+1}+x_{i})}{x_{i+2}-x_{i}} 
=\frac{x_{i+2}^3+x_{i+2}^2x_{i+1}+x_{i+2}x_{i+1}^2-x_{i+1}^2x_i-x_i^2x_{i+1}-x_i^3}{x_{i+2}-x_{i}} 
\]
$=$
\[
x^2_{i+2}+x_i^2+x_{i+2}x_i+x_{i+1}^2+x_{i+2}x_{i+1}+x_{i+1}x_i=\tau_{2}(x_i,x_{i+1},x_{i+2})
\]
\subsection*{VIII.b}
By the lemma of complete symmetric Polynomials:
\[
\tau_{k+1}(x_1,\ldots,x_n,x_{n+1})=
\tau_{k+1}(x_1,\ldots,x_n) + x_{n+1}\tau_{k}(x_1,\ldots,x_n,x_{n+1})
\]
We prove it by induction on n, when $n = 0$, it's trivial to see:
\[
\tau_m(x_i) = [x_i]x^m
\]
Assume when $n=k<m-1$ satisfies, suffices to show $n = k+1$ satisfies. Notice that:
\[
(x_{i+k+1}-x_1)\tau_{m-k-1}(x_{i},\ldots,x_{i+k+1}) = \tau_{m-k}(x_1,\ldots,x_{i+k},x_{i+k+1}) - 
\tau_{m-k}(x_1,\ldots,x_{i+k})-x_1\tau_{m-k-1}(x_{i},\ldots,x_{i+k+1})
\]
$=$
\[
 \tau_{m-k}(x_2,\ldots,x_{i+k},x_{i+k+1}) + x_1\tau_{m-k-1}(x_1,\ldots,x_{i+k},x_{i+k+1}) -
\tau_{m-k}(x_1,\ldots,x_{i+k})-x_1\tau_{m-k-1}(x_{i},\ldots,x_{i+k+1})
\]
$=$
\[
 \tau_{m-k}(x_2,\ldots,x_{i+k},x_{i+k+1})-
\tau_{m-k}(x_1,\ldots,x_{i+k}) = [x_2,\ldots,x_{i+k}]x^{m} - [x_1,\ldots,x_{i+k}]x^{m}
\]
$\Rightarrow$
\[
\tau_{m-k-1}(x_{i},\ldots,x_{i+k+1}) = \frac{[x_2,\ldots,x_{i+k+1}]x^{m} - [x_1,\ldots,x_{i+k}]x^{m}}{x_{i+k+1}-x_1}
= [x_1,\ldots,x_{i+k+1}]x^{m}
\]
